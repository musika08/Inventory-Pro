...
st.markdown(f"""
    ...
    hr {{ border: none !important; height: 1px !important; background-color: #333 !important; display: block !important; margin: 10px 0 !important; }}
    </style>
...
if st.sidebar.button("📊 Dashboard"): st.session_state.current_page = "Dashboard"
if st.sidebar.button("📂 Database"): st.session_state.current_page = "Database"
if st.sidebar.button("📦 Inventory"): st.session_state.current_page = "Inventory"
if st.sidebar.button("💰 Sales"): st.session_state.current_page = "Sales"
if st.sidebar.button("💸 Expenditures"): st.session_state.current_page = "Expenditures"
if st.sidebar.button("📜 Activity Log"): st.session_state.current_page = "Log"
...
if page == "Sales":
    ...
    if state["edited_rows"] or state["added_rows"] or state["deleted_rows"]:
        ...
        if state["deleted_rows"]:
            log_action(f"🔴 [REMOVED] Sales record deleted.")
        st.session_state.sales = new_df
        save_data(new_df, SALES_FILE)
        st.rerun()
...
# ---------------------------------------------------------
    ...
    m1, m2 = st.columns(2)
    e_total = st.session_state.expenditures['Cost'].sum()
    d_total = st.session_state.cash_in['Amount'].sum()
    m1.metric("Total Expenses", f"₱{e_total:,.2f}")
    m2.metric("Total Deposits", f"₱{d_total:,.2f}")

    st.write("### ➖ Log Expense")
    ex_row = st.columns([2, 1, 1, 0.4])
    ex_item = ex_row[0].text_input("Expense Item")
    ex_cost = ex_row[1].number_input("Cost (₱)", min_value=0.0)
    if ex_row[3].button("➕", key="btn_add_ex"):
        ...
        log_action(f"💸 [EXPENSE] '{ex_item}' added for ₱{ex_cost:,.2f}"); st.rerun()

    st.write("### ➕ Log Deposit")
    in_row = st.columns([2, 1, 1, 0.4])
    in_source = in_row[0].text_input("Source")
    in_amt = in_row[1].number_input("Amount (₱)", min_value=0.0)
    if in_row[3].button("➕", key="btn_add_in"):
        ...
        log_action(f"💰 [DEPOSIT] Received ₱{in_amt:,.2f} from '{in_source}'"); st.rerun()
    ...
    with l_c:
        ...
        if not ed_ex.equals(v_ex):
            log_action("🧹 [CLEANUP] Expense modified."); st.session_state.expenditures = ed_ex.iloc[::-1]; save_data(st.session_state.expenditures, EXPENSE_FILE); st.rerun()
    with r_c:
        ...
        if not ed_in.equals(v_in):
            log_action("🧹 [CLEANUP] Deposit modified."); st.session_state.cash_in = ed_in.iloc[::-1]; save_data(st.session_state.cash_in, CASH_FILE); st.rerun()
...
# ---------------------------------------------------------
    ...
    if os.path.exists(LOG_FILE):
        ...
        if st.button("🗑️ Clear Log"):
            os.remove(LOG_FILE); log_action("⚠️ Log cleared."); st.rerun()
...
# ---------------------------------------------------------
    ...
    if t_c2.button("➕"):
        if n_t and n_t not in db_df.columns:
            st.session_state.inventory[n_t] = 0.0
            save_data(st.session_state.inventory, DB_FILE); log_action(f"📂 [DATABASE] Added Price Tier '{n_t}'"); st.rerun()
    ed = st.data_editor(st.session_state.inventory, use_container_width=True, hide_index=True, num_rows="dynamic")
    if not ed.equals(st.session_state.inventory):
        log_action("📂 [DATABASE] modified."); st.session_state.inventory = ed; save_data(ed, DB_FILE); st.rerun()
...
# ---------------------------------------------------------
    ...
    with c_r:
        ...
        f_row = st.columns([1.5, 0.7, 1, 1, 0.4])
        n_p = f_row[0].selectbox("Product", product_list)
        n_q = f_row[1].number_input("Qty", min_value=1)
        n_s = f_row[2].selectbox("Status", ["In Stock", "Bought"])
        if f_row[4].button("➕"):
            n_row = pd.DataFrame({"Product Name": [n_p], "Quantity": [n_q], "Status": [n_s], "Date": [date.today()]})
            st.session_state.stock = pd.concat([st.session_state.stock, n_row], ignore_index=True); save_data(st.session_state.stock, STOCK_FILE)
            log_action(f"➕ [STOCK-IN] {n_q} units of '{n_p}'"); st.rerun()
        ...
        if not ed_s.equals(s_v):
            log_action("📦 [INVENTORY] Log adjusted."); st.session_state.stock = ed_s.iloc[::-1]; save_data(st.session_state.stock, STOCK_FILE); st.rerun()
...
# ---------------------------------------------------------
    ...
    with st.container(border=True):
        ...
        s_y = f_row[0].selectbox("Year", y)
        s_m = f_row[1].selectbox("Month", ["January", "February", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December"], index=date.today().month-1)
    
    month_idx = ["January", "February", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December"].index(s_m)+1
    f_sales = st.session_state.sales.copy()
    f_sales["Date"] = pd.to_datetime(f_sales["Date"])
    ...
    m3.metric("Net Period Profit", f"₱{(f_sales['Profit'].sum() - st.session_state.expenditures['Cost'].sum()):,.2f}")
    m4.metric("Unpaid Balance", f"₱{f_sales[f_sales['Payment'] == 'Unpaid']['Total'].sum():,.2f}")
    ...
    p = st.session_state.sales[['Date', 'Profit']].rename(columns={'Profit': 'Amt'})
    d = st.session_state.cash_in[['Date', 'Amount']].rename(columns={'Amount': 'Amt'})
    e = st.session_state.expenditures[['Date', 'Cost']].rename(columns={'Cost': 'Amt'})
    e['Amt'] = -e['Amt']
    t_df = pd.concat([p, d, e])
    if not t_df.empty:
        t_df['Date'] = pd.to_datetime(t_df['Date'])
        t_df = t_df.sort_values('Date').groupby('Date').sum().reset_index()
        t_df['CumCash'] = t_df['Amt'].cumsum()
